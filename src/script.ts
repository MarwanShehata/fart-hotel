/** biome-ignore-all lint/style/noNonNullAssertion: reduces a lot of noise */
/** biome-ignore-all lint/suspicious/useAwait: to be fixed when I implement the Firebase API */
import type {
  AudioState,
  AvatarStyle,
  BaseUserData,
...
export const getGuestUser = (): UserData => {
  ...
  if (storedUserJSON) {
    try {
      ...
      throw new Error(`Failed to parse guest user from storage: ${error}`)
    }
  }
...
class FartHotel {
  ...
  private initAudioContext(): void {
    if (!this.audioState.audioContext) {
      this.audioState.audioContext = new (
        // biome-ignore lint/suspicious/noExplicitAny: it's broken but works
  ...
  private handleGuess(e: MouseEvent): void {
    ...
    const guess = Number.parseInt(characterDiv.dataset.index!, 10)

    // Clear previous selections first
    for (const prevGuess of this.gameState.selectedGuesses) {
  ...
  private endRoundHandler(
    ...
    if (isCorrect) {
      ...
      if (this.gameState.score > this.gameState.highScore) {
        this.gameState.highScore = updateHighScore(
  ...
  private playBackgroundMusic(): void {
    ...
    this.audioState.backgroundMusicContext = new (
      // biome-ignore lint/suspicious/noExplicitAny: it's broken but works
      window.AudioContext || (window as any).webkitAudioContext
    )()
